...
ax.plot(ke, 10.*np.log10(RT.stot[pol]), label='STOT', color='k')
ax.plot(ke, 10.*np.log10(RT.s0g[pol]), label='SIGGROUND', color='r')
ax.plot(ke, 10.*np.log10(RT.s0c[pol]), label='SIG can', color='b')
# ax.plot(ke, 10.*np.log10(RT.s0cgt[pol]), label='SIG can ground', color='g')
# ax.plot(ke, 10.*np.log10(RT.s0gcg[pol]), label='SIG ground can ground', color='k', linestyle='--')
#ax.plot(theta_deg, 10.*np.log10(RT.G.rt_s.vv), label='surface', linestyle='-.')

ax.legend()
ax.set_title('s='+str(s)+' f='+str(f)+' sand='+str(0.4)+' clay='+str(0.3)+' bulk='+str(1.65)+' d='+str(d)+' mv='+str(0.2)+' theta='+str(theta_deg))


ax.grid()
ax.set_xlabel('ke [Np/m]')
ax.set_ylabel('sigma [dB]')
# ax.set_xlim(0.,70.)
# ax.set_ylim(-17.,-9.)
plt.savefig('/media/tweiss/Daten/plots/sensitivity/ke')
plt.close()
plt.show()

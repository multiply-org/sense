...
def warper(s_vv, m_vv, omega_vv, s_vh, m_vh, omega_vh):

    coef_vv = m_vv * CAB
    coef_vh = m_vh * CAB
    SAR = [f, theta]

    surface_vv    = [clay, sand, bulk, sm, s_vv]
    vegetation_vv = [1, LAI, coef_vv, omega_vv]


    surface_vh    = [clay, sand, bulk, sm, s_vh]
...
def cost(p):
    s_vv, m_vv, omega_vv, s_vh, m_vh, omega_vh = p
    #m_vv, m_vh = p[:len(LAI)], p[len(LAI):]
    #m_vv, m_vh = p
    vv, vh = warper(s_vv, m_vv, omega_vv, s_vh, m_vh, omega_vh)
...
pm_vv = 1.4
pm_vh = 0.8
bounds = [(0.001, 0.02), (0.1,7), (0.001,0.03), (0.001, 0.02), (0.1,7), (0.001,0.03)]

guess = ps_vv, pm_vv, pomega_vv, ps_vh, pm_vh, pomega_vh
#guess = [1.4, 0.8]
...
cost([0.00548515, 0.73296915, 0.02264721, 0.00776594, 0.32751273,
       0.001     ])
